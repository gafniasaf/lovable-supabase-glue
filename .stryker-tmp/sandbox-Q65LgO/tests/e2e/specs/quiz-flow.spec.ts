...
test.describe('Quiz end-to-end', () => {
	test('teacher creates quiz; student answers and submits; teacher sees attempts', async ({ request }) => {
		...
		const c1 = await request.post('/api/quiz-choices', { headers: { 'x-test-auth': 'teacher' }, data: { question_id: question.id, text: '3', is_correct: false, order_index: 1 } });
		expect(c1.ok()).toBeTruthy();
		const c2 = await request.post('/api/quiz-choices', { headers: { 'x-test-auth': 'teacher' }, data: { question_id: question.id, text: '4', is_correct: true, order_index: 2 } });
		...
		await request.patch('/api/quiz-attempts', { headers: { 'x-test-auth': 'student' }, data: { attempt_id: attempt.id, question_id: question.id, choice_text: '4' } });
