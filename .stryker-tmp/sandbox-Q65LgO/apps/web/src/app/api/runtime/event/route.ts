...
export const POST = withRouteTiming(async function POST(req: NextRequest) {
  ...
  // Authenticate runtime token (RS256 in prod; HS256 only in dev)
  ...
  try {
    ...
    if (pub) {
      ...
      const jose = await import('jose');
      const { payload } = await jose.jwtVerify(rt, secret, { algorithms: ['HS256'] });
      claims = payload;
    }
  } catch {
  ...
  // Optional audience binding and scope check: events.write
  try {
