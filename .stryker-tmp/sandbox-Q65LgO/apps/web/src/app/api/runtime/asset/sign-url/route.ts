...
export const POST = withRouteTiming(async function POST(req: NextRequest) {
  ...
  // Scope + content-type checks
  const scopes: string[] = Array.isArray((claims as any)?.scopes) ? (claims as any).scopes : [];
  if (!scopes.includes('files.write')) {
    return NextResponse.json({ error: { code: 'FORBIDDEN', message: 'Missing scope files.write' }, requestId }, { status: 403, headers: { 'x-request-id': requestId } });
  }
  const allowList = (process.env.ALLOWED_UPLOAD_MIME?.split(',').map(s => s.trim()).filter(Boolean)) || [
  ...
  return NextResponse.json({ url: signed.url, method: signed.method, headers: signed.headers, key: objectKey }, { status: 200, headers });
});
