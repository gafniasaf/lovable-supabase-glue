...
export const GET = withRouteTiming(async function GET(req: NextRequest) {
  ...
  try { return NextResponse.json((data ?? []).map(r => courseProvider.parse(r as any)), { status: 200, headers: { 'x-request-id': requestId } }); } catch { return NextResponse.json({ error: { code: 'INTERNAL', message: 'Invalid provider shape' }, requestId }, { status: 500, headers: { 'x-request-id': requestId } }); }
});
...
export const POST = withRouteTiming(async function POST(req: NextRequest) {
  ...
  try { return NextResponse.json(courseProvider.parse(data as any), { status: 201, headers: { 'x-request-id': requestId } }); } catch { return NextResponse.json({ error: { code: 'INTERNAL', message: 'Invalid provider shape' }, requestId }, { status: 500, headers: { 'x-request-id': requestId } }); }
});
...
export const PATCH = withRouteTiming(async function PATCH(req: NextRequest) {
  ...
  try { return NextResponse.json(courseProvider.parse(data as any), { status: 200, headers: { 'x-request-id': requestId } }); } catch { return NextResponse.json({ error: { code: 'INTERNAL', message: 'Invalid provider shape' }, requestId }, { status: 500, headers: { 'x-request-id': requestId } }); }
});
