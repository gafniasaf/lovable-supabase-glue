...
export const POST = withRouteTiming(async function POST(req: NextRequest) {
  ...
  try { return NextResponse.json(externalCourse.parse(data as any), { status: 201, headers: { "x-request-id": requestId } }); } catch { return NextResponse.json({ error: { code: 'INTERNAL', message: 'Invalid external course shape' }, requestId }, { status: 500, headers: { 'x-request-id': requestId } }); }
});
...
export const PATCH = withRouteTiming(async function PATCH(req: NextRequest) {
  ...
  try { return NextResponse.json(externalCourse.parse(data as any), { status: 200, headers: { "x-request-id": requestId } }); } catch { return NextResponse.json({ error: { code: 'INTERNAL', message: 'Invalid external course shape' }, requestId }, { status: 500, headers: { 'x-request-id': requestId } }); }
});
