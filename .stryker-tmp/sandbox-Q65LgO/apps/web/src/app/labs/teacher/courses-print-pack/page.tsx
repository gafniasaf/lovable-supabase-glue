...
async function fetchLessonsForCourse(courseId: string, init: RequestInit): Promise<Lesson[]> {
  const res = await serverFetch(`/api/lessons?course_id=${encodeURIComponent(courseId)}`, init);
  if (!res.ok) return [];
...
export default async function CoursesPrintPackPage({ searchParams }: { searchParams?: { ids?: string } }) {
  ...
  for (const id of courseIds) {
    const lessons = await fetchLessonsForCourse(id, commonInit);
