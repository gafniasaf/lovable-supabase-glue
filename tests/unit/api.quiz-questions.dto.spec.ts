...
const post = (body: any, headers?: Record<string,string>) => new Request('http://localhost/api/quiz-questions', { method: 'POST', headers: { 'content-type': 'application/json', ...(headers||{}) } as any, body: JSON.stringify(body) } as any);
const get = (url: string, headers?: Record<string,string>) => new Request(url, { method: 'GET', headers: headers as any } as any);

describe('quiz-questions DTO + headers', () => {
  ...
  test('POST returns x-request-id or guard status', async () => {
    ...
    const res = await (QQPost as any)(post({ quiz_id: '00000000-0000-0000-0000-000000000001', text: 'Q?', order_index: 1 }));
  ...
  test('GET returns x-request-id and JSON or guards', async () => {
    // @ts-ignore
    globalThis.__TEST_HEADERS_STORE__?.cookies?.set?.('x-test-auth', 'teacher');
    const res = await (QQGet as any)(get('http://localhost/api/quiz-questions?quiz_id=00000000-0000-0000-0000-000000000001'));
