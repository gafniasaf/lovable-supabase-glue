// Node variant of MSW shim that installs a global fetch interceptor
type Resolver = (req: Request) => Promise<Response> | Response;
const routes: { method: string; path: string; resolver: Resolver }[] = [];

function normalizePath(input: string): string {
  try { const u = new URL(input); return u.pathname; } catch { return input; }
}

function installFetchInterceptor() {
  const g: any = globalThis as any;
  const originalFetch: typeof fetch | undefined = g.fetch;
  // Ensure fetch/Response/Request exist using undici when missing
  try {
  ...
  g.fetch = async (input: any, init?: RequestInit) => {
    try {
      ...
      const hit = routes.find(r => r.method === method && (r.path === path || url.endsWith(r.path) || url.includes(`${r.path}?`)));
      if (hit) {
    ...
    if (typeof Resp !== 'function') {
      try { const u = require('undici'); Resp = u.Response; } catch { Resp = SimpleResponse as any; }
    }
    return new Resp(JSON.stringify({ ok: false }), { status: 404, headers: { 'content-type': 'application/json' } });
  };
}

export function setupServer(...defs: any[]) {
  // Pre-register any initial handlers
  for (const d of defs) {
