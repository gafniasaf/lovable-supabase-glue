...
export default function DataTable<T extends { id?: string | number }>({ columns, rows, empty }: { columns: Column<T>[]; rows: T[]; empty?: React.ReactNode }) {
	...
	return (
		...
		<table className="data-table w-full text-sm border">
			<thead>
				<tr className="bg-gray-50 text-left">
					{columns.map(c => (
						<th
							...
							className={"sticky top-0 z-10 bg-white dark:bg-gray-900 p-2 border cursor-pointer select-none " + (c.className || '')}
			...
			<tbody>
				{(sorted || []).map((r, idx) => (
					<tr key={(r as any).id ?? idx} className="border-b odd:bg-gray-50 dark:odd:bg-gray-800">
