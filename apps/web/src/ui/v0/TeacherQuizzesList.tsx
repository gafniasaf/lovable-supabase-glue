...
export type TeacherQuizzesListProps = {
  header?: { title?: string };
...
export default function TeacherQuizzesList({ header = { title: "Quizzes" }, actions = [], items = [], state = "default" }: TeacherQuizzesListProps) {
  if (state === "loading") {
    return (
  ...
  return (
    <section className="p-6">
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <div className="bg-card rounded-lg border divide-y" data-testid="quizzes-list">
          {items.map((q) => (
            <div key={q.id} data-testid="quiz-row" className="p-4 flex items-center justify-between hover:bg-muted/30">
              <div className="font-medium" data-testid="quiz-title">{q.title}</div>
              {q.href && <a href={q.href} className="text-sm underline">Manage</a>}
            </div>
          ))}
        </div>
      </div>
