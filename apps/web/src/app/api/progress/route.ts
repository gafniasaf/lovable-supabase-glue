...
export const GET = withRouteTiming(async function GET(req: NextRequest) {
	...
	if (isTestMode()) {
		if (perStudent) {
			return jsonDto({ students: [] } as any, z.object({ students: z.array(z.any()) }) as any, { requestId, status: 200 });
		}
		return jsonDto({} as any, z.object({}).passthrough() as any, { requestId, status: 200 });
	}
	...
	if (ids.length === 0) return jsonDto((perStudent ? { students: [] } : {}) as any, (perStudent ? z.object({ students: z.array(z.any()) }) : z.object({}).passthrough()) as any, { requestId, status: 200 });
	...
	if (perStudent) {
		...
		if (studentIds.length === 0) return jsonDto({ students: [] } as any, z.object({ students: z.array(z.any()) }) as any, { requestId, status: 200 });
		...
		return jsonDto({ students } as any, z.object({ students: z.array(z.object({ student_id: z.string(), completedLessons: z.number().int().nonnegative(), totalLessons: z.number().int().nonnegative(), percent: z.number().int().min(0).max(100), name: z.string().min(1) })) }) as any, { requestId, status: 200 });
	}
	...
	if (forTeacher && isTeacher) {
		...
		return jsonDto({ counts } as any, z.object({ counts: z.record(z.number().int().nonnegative()) }) as any, { requestId, status: 200 });
	} else {
		const map: Record<string, true> = {};
		for (const row of data ?? []) map[(row as any).lesson_id] = true;
		return jsonDto(map as any, z.record(z.boolean()) as any, { requestId, status: 200 });
	}
});
