...
export const GET = withRouteTiming(async function GET(req: NextRequest) {
  ...
  const dto = z.object({ url: z.string().url(), filename: z.string().nullable(), content_type: z.string().nullable() });
  return jsonDto({ url, filename: att.filename ?? null, content_type: att.content_type ?? null } as any, dto as any, { requestId, status: 200 });
});
