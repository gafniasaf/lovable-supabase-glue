...
export const POST = withRouteTiming(createApiHandler({
  ...
  handler: async (input, ctx) => {
    ...
    if (isTestMode()) {
      ...
      try {
        try { await recordEvent({ user_id: user.id, event_type: 'enrollment.create', entity_type: 'course', entity_id: course_id }); } catch {}
        return NextResponse.json(enrollment.parse(row), { status: 201 });
      } catch { return NextResponse.json({ error: { code: 'INTERNAL', message: 'Invalid enrollment shape' }, requestId }, { status: 500, headers: { 'x-request-id': requestId } }); }
    }
    ...
    try {
      try { await recordEvent({ user_id: user.id, event_type: 'enrollment.create', entity_type: 'course', entity_id: course_id }); } catch {}
      return jsonDto(enrollment.parse(data as any), enrollment as any, { requestId, status: 201 });
    } catch { return NextResponse.json({ error: { code: 'INTERNAL', message: 'Invalid enrollment shape' }, requestId }, { status: 500, headers: { 'x-request-id': requestId } }); }
  }
}));
...
export const GET = withRouteTiming(async function GET(req: NextRequest) {
  ...
  if (isTestMode()) {
    ...
    try {
      ...
      const res = jsonDto(parsed, enrollmentListDto as any, { requestId, status: 200 });
