...
export const GET = withRouteTiming(async function GET(req: Request) {
  ...
  if (ids.length === 0) return jsonDto([] as any, z.array(z.object({ id: z.string().min(1), display_name: z.string().nullable().optional(), email: z.string().email().nullable().optional() })) as any, { requestId, status: 200 });
