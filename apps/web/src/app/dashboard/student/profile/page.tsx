...
export default async function StudentProfilePage({ searchParams }: { searchParams?: { [k: string]: string | string[] | undefined } }) {
  ...
  async function updateAction(formData: FormData) {
    ...
    try {
      ...
      redirect('/dashboard/student/profile?ok=1');
    } catch {
      const { redirect } = await import('next/navigation');
      redirect('/dashboard/student/profile?error=1');
    }
  }

  return (
    <section className="p-6 space-y-4" aria-label="Student profile">
