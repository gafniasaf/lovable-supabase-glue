...
export default async function TeacherDashboard() {
  ...
  return (
    <section className="p-6" aria-label="Teacher dashboard">
      ...
      {/* Recently graded panel for quick context reversal */}
      <section className="mt-4 border rounded p-4">
        <h2 className="font-medium">Recently graded</h2>
        <ul className="mt-2 text-sm space-y-1">
          {(notifications || []).filter((n: any) => n.type === 'submission:graded').slice(0,5).map((n: any) => (
            <li key={n.id} className="flex items-center justify-between">
              <span>{n.payload?.student_id || 'Student'} · {n.payload?.assignment_id || 'Assignment'}</span>
              <span className="text-gray-600">{n.payload?.score != null ? `Score ${n.payload.score}` : '-'}</span>
              <span className="text-gray-500">{new Date(n.created_at).toLocaleString()}</span>
            </li>
          ))}
          {(!notifications || (notifications.filter((n: any) => n.type === 'submission:graded').length === 0)) && (
            <li className="text-gray-500">No recent grades</li>
          )}
        </ul>
      </section>
