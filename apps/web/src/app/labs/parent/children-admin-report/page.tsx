...
export default async function ParentChildrenAdminReportPage({ searchParams }: { searchParams?: { q?: string | string[] } }) {
  const h = headers();
  const c = cookies();
  ...
  const csvHref = `data:text/csv;charset=utf-8,${encodeURIComponent(toCsv(filtered))}`;

  return (
    <main className="p-6 space-y-4">
      <h1 className="text-xl font-semibold">Children admin report</h1>
