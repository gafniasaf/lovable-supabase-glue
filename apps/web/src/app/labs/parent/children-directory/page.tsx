...
export default async function ParentChildrenDirectoryPage({ searchParams }: { searchParams?: { q?: string | string[] } }) {
  const h = headers();
  const c = cookies();
  ...
  try {
    allRows = await createParentLinksGateway().listByParent('test-parent-id') as any;
  } catch {}
  ...
  const rows = q ? allRows.filter(r => r.student_id.includes(q)) : allRows;
  const total = rows.length;
  const csvHref = `data:text/csv;charset=utf-8,${encodeURIComponent(toCsv(rows))}`;

  return (
    <main className="p-6 space-y-4">
      <h1 className="text-xl font-semibold">Children directory</h1>
        ...
        <ul className="space-y-2" data-testid="children-list">
          {rows.map(row => (
            <li key={row.id} data-testid="child-row">
              <a className="text-blue-600 underline" data-testid="child-student-id" href={`/labs/parent/children-directory/${encodeURIComponent(row.student_id)}/print`}>{row.student_id}</a>
            </li>
          ))}
        </ul>
      )}
