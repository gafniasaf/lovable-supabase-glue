...
async function getStudentDashboard(userId: string) {
  ...
  logger.info({ ms: Date.now() - t0 }, "dash_student_widgets");
  // Compute total lessons completed KPI
  let lessonsCompletedTotal = 0;
  for (const cid of courseIds) {
    const lessons = lessonsByCourse.get(cid) ?? [];
    for (const l of lessons) if (progressSet.has(l.id)) lessonsCompletedTotal += 1;
  }
  return dashboardResponse.parse({
    role: "student",
    data: {
      kpis: { enrolledCourses: { label: "Courses", value: enrollments.length }, lessonsCompleted: { label: "Lessons", value: lessonsCompletedTotal } },
      continueLearning,
