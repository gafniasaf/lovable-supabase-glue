...
export function ensureJobsStarted() {
  if (started) return;
  started = true;
  // Only enable in production-like environments when explicitly opted in
  if (process.env.DUE_SOON_JOB !== '1') return;
  const intervalMs = Number(process.env.DUE_SOON_INTERVAL_MS || 5 * 60 * 1000);
  scheduleJob('assignment_due_soon', intervalMs, async () => {
  ...
  // Data retention: optional cleanup for old notifications and read receipts
  if (process.env.DATA_RETENTION_JOB === '1') {
    ...
    scheduleJob('data_retention_cleanup', cleanupInterval, async () => {
      const supabase = getRouteHandlerSupabase();
      ...
      try {
        ...
        for (const att of (atts ?? []) as any[]) {
          ...
          if (!referenced) {
            try {
              // announcements / lessons file_key references
  ...
  // Provider health refresh job (best-effort caching)
  if (process.env.PROVIDER_HEALTH_REFRESH_JOB === '1') {
    ...
    scheduleJob('provider_health_refresh', interval, async () => {
      const supabase = getRouteHandlerSupabase();
  ...
  // Progress MV refresh job: keep user_course_progress_summary reasonably fresh
  if (process.env.REFRESH_PROGRESS_SUMMARY_JOB === '1') {
    ...
    scheduleJob('refresh_progress_summary', interval, async () => {
      const supabase = getRouteHandlerSupabase();
      try {
      ...
      try {
        // Fallback: issue a dummy select to trigger a refresh via SQL function if exposed
        await (supabase as any).rpc?.('refresh_user_course_progress_summary');
      } catch {}
    });
  }

  // Quota reconciliation job: sum attachments.size_bytes per user and update used_bytes
  if (process.env.QUOTA_RECONCILE_JOB === '1') {
    const interval = Number(process.env.QUOTA_RECONCILE_INTERVAL_MS || 6 * 60 * 60 * 1000);
    scheduleJob('quota_reconcile', interval, async () => {
      const supabase = getRouteHandlerSupabase();
      ...
      for (const q of (quotas ?? []) as any[]) {
        ...
        try {
          ...
          // Fallback to client-side sum if no RPC
          let sum = 0;
  ...
  // Attachment size backfill: populate size_bytes for attachments missing it and adjust quotas
  if (process.env.BACKFILL_ATTACHMENT_SIZES_JOB === '1') {
    ...
    scheduleJob('attachment_size_backfill', interval, async () => {
      const supabase = getRouteHandlerSupabase();
