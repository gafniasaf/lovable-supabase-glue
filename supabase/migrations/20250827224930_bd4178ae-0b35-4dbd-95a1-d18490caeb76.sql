-- Phase 2: Enhanced Grade Management - Database Schema Updates (Fixed)
...
-- Create grade history table for tracking grade changes (only if not exists)
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'grade_history' AND table_schema = 'public') THEN
